...
// Base map selection: OpenFreeMap is the default; Esri World Imagery is an optional switch.
// OpenFreeMap uses OpenStreetMap data and its public instance is free with no API key.
const openFreeMapLayer = L.maplibreGL({
  style: "https://tiles.openfreemap.org/styles/liberty",
  attribution: "OpenFreeMap © OpenMapTiles Data from OpenStreetMap"
}).addTo(map);

// Keep the original Esri World Imagery as a selectable base map, without the street-label overlay.
...
let activeBaseMap = "openfreemap";

function setBaseMap(name) {
  if (name === "esri") {
    if (map.hasLayer(openFreeMapLayer)) map.removeLayer(openFreeMapLayer);
    if (!map.hasLayer(esriImageryLayer)) esriImageryLayer.addTo(map);
    activeBaseMap = "esri";
  } else {
    if (map.hasLayer(esriImageryLayer)) map.removeLayer(esriImageryLayer);
    if (!map.hasLayer(openFreeMapLayer)) openFreeMapLayer.addTo(map);
...
baseMapControl.onAdd = () => {
  ...
  wrap.querySelector("#baseMapPreview").addEventListener("click", () => {
    setBaseMap(activeBaseMap === "openfreemap" ? "esri" : "openfreemap");
  });
  return wrap;
};
...
function updateBaseMapPreview() {
  ...
  if (!thumb) return;
  thumb.classList.toggle("esri", activeBaseMap === "esri");
}
updateBaseMapPreview();
...
const handleSearch = () => { rebuild(); s11Update(); cjUpdate(); };
