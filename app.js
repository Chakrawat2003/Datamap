...
function s11PopupHtml(rec, opts = {}) {
  ...
  const editBtn = opts.custom || opts.editable
    ? `<button type="button" class="pin-nav-link s11EditBtn" data-id="${esc(rec._localId ?? rec.id ?? "")}" data-custom="${opts.custom ? "1" : "0"}" style="background:#111">✎ แก้ไข</button>`
    : "";
...
const S11_LOGO_URL = "7-11-logo.png";

function s11MakeIcon(color = "green") {
  const border = s11SafeColor(color);
  const html = `<div class="s11-marker-logo" style="--s11-border:${border}">
    <img src="${S11_LOGO_URL}" alt="7-Eleven" draggable="false">
  </div>`;
  return L.divIcon({
...
function s11BuildAll() {
  const base = (window.SEVEN11_DATA || [])
    .filter(rec => !S11.deletedIds.has(String(rec.id)))
    .map(rec => {
      const edit = S11.edits.get(String(rec.id));
      const merged = edit ? { ...rec, ...edit } : rec;
      const m = L.marker([merged.y, merged.x], { icon: s11MakeIcon(merged.color) });
      m.bindPopup(s11PopupHtml(merged, { editable: true }));
      return { rec: merged, m, id: rec.id, isCustom: false };
    });
  const custom = S11.custom.map(rec => {
    const m = L.marker([rec.y, rec.x], { icon: s11MakeIcon(rec.color) });
    m.bindPopup(s11PopupHtml(rec, { custom: true }));
    return { rec, m, id: rec._localId, isCustom: true };
  });
...
// 7-Eleven popup edit buttons: use document delegation so mouse + touch work reliably.
document.addEventListener("click", e => {
  ...
  if (isCustom) {
    const rec = S11.custom.find(r => String(r._localId) === id);
    if (rec) openS11Modal(rec, null, true);
  } else {
    const base = (window.SEVEN11_DATA || []).find(r => String(r.id) === id);
    const edit = S11.edits.get(String(id));
    if (base) openS11Modal({ ...base, ...edit }, null, false, id);
  }
}, true);
...
function s11PopulateColors() {
  ...
  wrap.innerHTML = colors.map(([name, hex, label]) =>
    `<button type="button" class="swatch s11ColorSwatch" data-color="${name}" title="${label}" aria-label="${label}">
      <img src="${S11_LOGO_URL}" alt="${label}" draggable="false">
    </button>`
  ).join("");
...
function closeS11Modal() { $("s11Modal")?.classList.add("hidden"); }
if ($("s11Close")) {
  ["pointerdown", "click"].forEach(type => $("s11Close").addEventListener(type, e => {
    e.preventDefault();
    e.stopPropagation();
    closeS11Modal();
  }));
...
if ($("s11Save")) {
  $("s11Save").onclick = () => {
    ...
    if (ctx.rec && ctx.isCustomEdit) {
      ...
      S11.custom.push({ _localId: "c_" + Date.now(), n, a, note, color, y: ll.lat, x: ll.lng });
    }
    s11Persist();
    S11.all = [];
    s11Update();
...
function pinIcon(color, icon, iconImg) {
  if (icon === "photo" && iconImg) {
    return L.divIcon({ className: "", html: `<img src="${iconImg}" style="width:32px;height:32px;border-radius:50%;border:2px solid #fff;box-shadow:0 2px 6px #0006;object-fit:cover">`, iconSize: [36, 36], iconAnchor: [18, 32] });
  }
...
if ($("mProfileBtn")) $("mProfileBtn").onclick = () => { $("mobileMenu")?.classList.add("hidden"); openProfileOrAuth(); };
...
if ($("mLogoutBtn")) $("mLogoutBtn").onclick = () => { $("mobileMenu")?.classList.add("hidden"); doLogout(); };
...
if ($("mMenuBtn")) {
  $("mMenuBtn").addEventListener("click", e => {
    e.preventDefault();
    e.stopPropagation();
...
if ($("mobileMenuClose")) {
  ["pointerdown", "click"].forEach(type => {
    $("mobileMenuClose").addEventListener(type, e => {
      e.preventDefault();
      e.stopPropagation();
      closeMobileMenu();
    });
...
if ($("mTableToggle")) $("mTableToggle").onclick = () => { $("mobileMenu")?.classList.add("hidden"); doTableToggle(); };
...
if ($("mExportXlsx")) $("mExportXlsx").onclick = () => { $("mobileMenu")?.classList.add("hidden"); doExportXlsx(); };
...
if ($("mShare")) $("mShare").onclick = () => { $("mobileMenu")?.classList.add("hidden"); doShare(); };
...
function renderSavedRooms() {
  ["savedRooms", "mSavedRooms"].forEach(id => {
    ...
    wrap.querySelectorAll(".savedRoomBtn").forEach(b => {
      b.onclick = () => { $("mobileMenu")?.classList.add("hidden"); enterSavedRoom(b.dataset.room); };
    });
  });
}
...
if ($("mSaveRoom")) $("mSaveRoom").onclick = () => { $("mobileMenu")?.classList.add("hidden"); doSaveRoom(); };
...
// Close overlays reliably on mobile/desktop.
document.addEventListener("keydown", e => {
  if (e.key !== "Escape") return;
  closeMobileMenu();
  $("tablePanel")?.classList.add("hidden");
  $("modal")?.classList.add("hidden");
  $("s11Modal")?.classList.add("hidden");
  $("authModal")?.classList.add("hidden");
  $("profileModal")?.classList.add("hidden");
});

["modal", "s11Modal", "authModal", "profileModal"].forEach(id => {
  const el = $(id);
  if (!el) return;
...
async function init() {
  ...
  refreshUserUI();
  if (!S.session) openAuth("กรุณาเข้าสู่ระบบเพื่อเริ่มใช้งาน");
  else onAuthReady();
