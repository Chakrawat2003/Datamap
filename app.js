...
// Primary: MapProxy WMTS (dol_parcel / EPSG:3857).
// Fallback: the original DOL WMS endpoint.
// Last fallback: the DOL ArcGIS REST cached tiles.
// The source is switched automatically when the current source returns
// repeated tile errors before any tile has successfully loaded.
const DOL_MAP_PROXY_WMTS = "https://row.drr.go.th/mapproxy/wmts/dol_parcel/GLOBAL_WEBMERCATOR/{z}/{x}/{y}.png";
const DOL_OLD_WMS = "https://landsgis.dol.go.th/arcgis/services/DOL_PARCEL/MapServer/WMSServer";
const DOL_OLD_REST = "https://landsgis.dol.go.th/arcgis/rest/services/DOL_PARCEL/MapServer";

const dolLayers = [
  L.tileLayer(DOL_MAP_PROXY_WMTS, {
    minZoom: 0,
    maxZoom: 22,
    maxNativeZoom: 22,
    tileSize: 256,
    ...
    keepBuffer: 2,
    attribution: "กรมที่ดิน (DOL) / MapProxy"
  }),
  L.tileLayer.wms(DOL_OLD_WMS, {
    layers: "0",
    format: "image/png",
    transparent: true,
    maxZoom: 20,
  ...
  L.tileLayer(`${DOL_OLD_REST}/tile/{z}/{y}/{x}`, {
    minZoom: 0,
    maxZoom: 20,
    maxNativeZoom: 19,
    tileSize: 256,
...
const dolSourceNames = ["MapProxy WMTS", "DOL WMS เดิม", "DOL REST เดิม"];
let dolOn = false;
let dolSourceIndex = 0;
...
function dolRemoveAllLayers() {
  dolLayers.forEach(layer => {
    if (map.hasLayer(layer)) map.removeLayer(layer);
  });
}

function dolActivateSource(index, showToast = true) {
  if (!dolOn) return;
  dolClearFallbackTimer();
  dolRemoveAllLayers();

  dolSourceIndex = index;
  dolTileLoaded = false;
  dolTileErrors = 0;

  const layer = dolLayers[dolSourceIndex];
  if (!layer) {
    toast("ไม่สามารถโหลดข้อมูล DOL จากทุกแหล่งได้");
    return;
  }

  layer.addTo(map);
  if (showToast) toast(`กำลังโหลด DOL (${dolSourceNames[dolSourceIndex]})...`);

  // Give the current source a few seconds to return at least one tile.
  // If it only returns errors, automatically move to the next source.
  dolFallbackTimer = setTimeout(() => {
    dolFallbackTimer = null;
    if (!dolOn || dolTileLoaded) return;
    if (dolSourceIndex < dolLayers.length - 1) {
      toast(`DOL แหล่ง ${dolSourceNames[dolSourceIndex]} ใช้งานไม่ได้ — กำลังลองแหล่งสำรอง`);
      dolActivateSource(dolSourceIndex + 1);
    } else {
      toast("ไม่สามารถโหลดข้อมูล DOL ได้จากทุกแหล่ง");
    }
  }, 6500);
}

dolLayers.forEach((layer, index) => {
  layer.on("tileload", () => {
    // Ignore late events from a source that is no longer active.
    if (!dolOn || index !== dolSourceIndex) return;
    dolTileLoaded = true;
    dolTileErrors = 0;
    dolClearFallbackTimer();
    toast(`เปิดชั้นข้อมูลแปลงที่ดิน (DOL) แล้ว — ${dolSourceNames[index]}`);
  });
  ...
  layer.on("tileerror", () => {
    ...
    // Don't switch on the first failed tile: a viewport can request
    // several tiles and one transient failure should not trigger fallback.
    if (!dolTileLoaded && dolTileErrors >= 3 && dolSourceIndex < dolLayers.length - 1) {
      dolClearFallbackTimer();
      toast(`DOL แหล่ง ${dolSourceNames[index]} ตอบกลับผิดพลาด — กำลังลองแหล่งสำรอง`);
      dolActivateSource(dolSourceIndex + 1);
    }
  });
});
