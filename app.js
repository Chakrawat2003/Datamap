...
let mobileCopyTimer = null;
let mobileCopyStart = null;
...
// Mobile: press and hold on the map to show the same Copy popup.
const mapContainer = map.getContainer();
mapContainer.addEventListener("touchstart", e => {
  if (!e.touches || e.touches.length !== 1) return;
  const t = e.touches[0];
  mobileCopyStart = { x: t.clientX, y: t.clientY };
  clearTimeout(mobileCopyTimer);
  mobileCopyTimer = setTimeout(() => {
    if (!mobileCopyStart) return;
    const rect = mapContainer.getBoundingClientRect();
    const point = L.point(
      mobileCopyStart.x - rect.left,
      mobileCopyStart.y - rect.top
    );
    showCoordinateCopy(map.containerPointToLatLng(point));
    mobileCopyStart = null;
  }, 650);
}, { passive: true });

mapContainer.addEventListener("touchmove", e => {
  if (!mobileCopyStart || !e.touches || e.touches.length !== 1) return;
  const t = e.touches[0];
  const dx = t.clientX - mobileCopyStart.x;
  const dy = t.clientY - mobileCopyStart.y;
...
const PINMAP_VIEWPORT_PADDING = 0.20;
...
(function ensureSmallCompetitorMarkers(){
  ...
  st.textContent = `.s11-logo-marker,.cjmore-logo-marker{width:25px !important;height:25px !important;display:flex !important;align-items:center !important;justify-content:center !important;overflow:hidden !important;box-sizing:border-box !important;} .s11-logo-marker img,.cjmore-logo-marker img{width:25px !important;height:25px !important;display:block !important;object-fit:contain !important;}`;
...
function s11BuildAll() {
  const base = (window.SEVEN11_DATA || []).map((rec, index) => {
    ...
    const m = L.marker(
      [merged.y, merged.x],
      {
        icon: s11MakeIcon(merged.color || "green")
      }
    );

    m.bindPopup(
      s11PopupHtml(merged, { editable: true }),
      {
        closeButton: true,
        autoClose: true,
        closeOnClick: false
    ...
    m.on("popupopen", ev => {
      ...
      const btn =
        ev.popup.getElement()?.querySelector(".s11EditBtn");
...
function s11Update() {
  ...
  const visibleList = list.filter(x => pinmapInViewport(x.rec.y, x.rec.x, bounds));

  if (visibleList.length) {
    visibleList.forEach(x => s11Cluster.addLayer(x.m));
  }

  if (
    visibleList.length &&
    !map.hasLayer(s11Cluster)
  ) {
    s11Cluster.addTo(map);
  }
...
function cjUpdate() {
  ...
  const bounds = pinmapVisibleBounds();
  const visibleList = list.filter(x => pinmapInViewport(x.rec.lat, x.rec.lng, bounds));

  if (visibleList.length) {

    visibleList.forEach(x => cjCluster.addLayer(x.m));
  }

  if (
    visibleList.length &&
    !map.hasLayer(cjCluster)
  ) {
...
function rebuild() {
  ...
  const bounds = pinmapVisibleBounds();
  filtered.forEach(p => {
    if (pinmapInViewport(p.lat, p.lng, bounds)) marker(p);
  });
...
async function loadCompetitorDataFromSupabase() {
  ...
  try {
    ...
    console.error(
      "Competitor Supabase load error:",
      err
    );

    toast(
      "โหลด 7-Eleven / CJ MORE จาก Supabase ไม่สำเร็จ"
    );
  }
}
/* =========================================================
  ...
  competitorRealtimeChannel = S.sb
    ...
    .on(
      ...
      async (payload) => {
        ...
        try {
          /* Realtime ไม่ต้องยิงโหลด 2 ตารางซ้ำทุก event */
          scheduleCompetitorReload(250);
        } catch (err) {
          console.error("[Realtime] 7-11 reload error:", err);
        }
      }
    )
    ...
    .on(
      ...
      async (payload) => {
        ...
        try {
          ...
          console.error("[Realtime] CJMore reload error:", err);
        }
      }
    )
...
// Re-attach only markers that are inside/near the current viewport after pan/zoom.
// Data stays loaded; only the markers currently needed on screen are attached to Leaflet.
...
function pinmapRefreshViewport() {
  ...
  pinmapViewportTimer = setTimeout(() => {
    ...
    // User pins: remove off-screen markers and create only newly visible ones.
    const q = (($("search") ? $("search").value : "") || ($("mSearchInput") ? $("mSearchInput").value : "")).toLowerCase().trim();
    const filteredPins = S.all.filter(p =>
      (!q || (p.title + " " + (p.address || "") + " " + (p.note || "")).toLowerCase().includes(q))
    );
    for (const [id, m] of S.markers) {
      const p = S.all.find(x => String(x.id) === String(id));
      if (!p || !pinmapInViewport(p.lat, p.lng, bounds) || !filteredPins.includes(p)) {
        cluster.removeLayer(m);
        S.markers.delete(id);
      }
    }
    filteredPins.forEach(p => {
      if (pinmapInViewport(p.lat, p.lng, bounds) && !S.markers.has(p.id)) marker(p);
    });

    // Competitors: reuse existing marker objects; do not rebuild thousands of markers on every move.
    if (S11.all && S11.all.length) {
      const search = q;
      const list = search
        ? S11.all.filter(x => (String(x.rec.n || "") + " " + String(x.rec.a || "")).toLowerCase().includes(search))
        : (S11.on ? S11.all : []);
      s11Cluster.clearLayers();
      list.forEach(x => {
        if (pinmapInViewport(x.rec.y, x.rec.x, bounds)) s11Cluster.addLayer(x.m);
      });
      if (list.length && map.hasLayer(s11Cluster) === false) s11Cluster.addTo(map);
      if (!list.length && map.hasLayer(s11Cluster)) map.removeLayer(s11Cluster);
    }

    if (CJ.all && CJ.all.length) {
      const search = q;
      const list = search
        ? CJ.all.filter(x => (cjDisplayName(x.rec) + " " + (x.rec.address || "") + " " + (x.rec.description || "")).toLowerCase().includes(search))
        : (CJ.on ? CJ.all : []);
      cjCluster.clearLayers();
      list.forEach(x => {
        if (pinmapInViewport(x.rec.lat, x.rec.lng, bounds)) cjCluster.addLayer(x.m);
      });
      if (list.length && map.hasLayer(cjCluster) === false) cjCluster.addTo(map);
      if (!list.length && map.hasLayer(cjCluster)) map.removeLayer(cjCluster);
    }
  }, 120);
}
map.on("moveend", pinmapRefreshViewport);
