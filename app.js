...
async function sendPasswordReset() {
  ...
  if (!email) {
    if ($("authMessage")) $("authMessage").textContent = "กรุณากรอกอีเมลก่อนกดลืมรหัสผ่าน";
    $("authEmail")?.focus();
    return;
  }
  if (!/^\S+@\S+\.\S+$/.test(email)) {
    if ($("authMessage")) $("authMessage").textContent = "รูปแบบอีเมลไม่ถูกต้อง";
    $("authEmail")?.focus();
    return;
  }
  ...
  try {
    ...
    if ($("authMessage")) {
      $("authMessage").style.color = "#008450";
      $("authMessage").textContent = "ส่งลิงก์ตั้งรหัสผ่านใหม่ไปที่อีเมลแล้ว กรุณาตรวจสอบกล่องจดหมาย";
    }
  } catch (err) {
    if ($("authMessage")) {
      $("authMessage").style.color = "#ef4444";
      $("authMessage").textContent = err.message || "ส่งลิงก์ลืมรหัสผ่านไม่สำเร็จ";
    }
  } finally {
...
function openResetModal() {
  ...
  $("resetPasswordConfirm") && ($("resetPasswordConfirm").value = "");
...
async function saveNewPassword() {
  ...
  if (password.length < 6) {
    if ($("resetMessage")) $("resetMessage").textContent = "รหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร";
    return;
  }
  if (password !== confirm) {
    if ($("resetMessage")) $("resetMessage").textContent = "รหัสผ่านทั้งสองช่องไม่ตรงกัน";
    return;
  }
  ...
  try {
    ...
    if ($("resetMessage")) $("resetMessage").textContent = err.message || "เปลี่ยนรหัสผ่านไม่สำเร็จ";
  } finally {
    if (btn) btn.disabled = false;
  }
}

if ($("forgotPassword")) $("forgotPassword").onclick = sendPasswordReset;
...
// Supabase password-recovery links return to this page with a recovery session.
if (S.sb) {
