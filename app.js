...
if ($("save")) {
  $("save").onclick = async () => {
    ...
    const payload = {
      ...
      lat: S.pending.lat,
      lng: S.pending.lng,
      // Preserve original pinner attribution forever even if that person later leaves the room.
      created_by: S.editing?.created_by || username(),
      created_by_user_id: S.editing?.created_by_user_id || S.session.user.id
    };
    $("save").disabled = true;
    try {
      if (S.editing) {
...
function renderTable(rows = S.all) {
  ...
  body.innerHTML = `<table class="pinTable"><thead><tr><th>ชื่อจุด</th><th>จุดที่</th><th>Address</th><th>Area</th><th>สี</th><th>ชื่อโลโก้</th><th>ปักโดย</th></tr></thead><tbody>
    ${pinmapTableRows.map(p => `<tr data-id="${p.id}"><td>${esc(p.title)}</td><td>${esc(p.point_no || "")}</td><td>${esc(p.address || "")}</td><td>${esc(p.area || "")}</td><td><span style="display:inline-block;width:14px;height:14px;border-radius:50%;vertical-align:middle;background:${esc(p.color || "#ef4444")}"></span></td><td>${esc(pinStatusName(p.icon))}</td><td>${esc(p.created_by || "")}</td></tr>`).join("")}
  </tbody></table>`;
...
(function setupPinmapDashboard(){
  ...
  const DASH_ICON_NAMES = {
    pin:"Survey",decision:"Decision",star:"Star",heart:"Heart",flag:"Flag",home:"Home",food:"Food",
    shop:"Shop",hotel:"Hotel",photo:"Photo",van:"Van",m1:"M1",x:"X",deal:"Deal",cjx:"CJX"
  };

  let dashRows=[], dashData=[], dashRoom="", dashLoadedAt=0;
  const DASH_CACHE_MS = 5 * 60 * 1000;
  const DASH_PIN_SELECT = "id,lat,lng,area,icon,color,point_no";
  let dashProvince="", dashDistrict="", dashTambon="", dashIcon="", dashArea="", dashPointNo="";
  ...
  st.textContent=`
    ...
    #pinmapDashboardPanel{
      position:absolute;z-index:1450;right:14px;top:86px;bottom:16px;width:560px;
    ...
    .pmd-filters{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:7px;margin-bottom:12px}
    ...
    .pmd-icon-card{border:1px solid #e5e7eb;background:#fff;border-radius:11px;padding:8px;display:flex;align-items:center;gap:8px;cursor:pointer;min-width:0;text-align:left}
    .pmd-icon-card.active{outline:2px solid #111}
    .pmd-icon-pic{width:30px;height:30px;border-radius:8px;background:#f3f4f6;display:flex;align-items:center;justify-content:center;overflow:hidden;flex:0 0 auto;font-size:18px}
    .pmd-icon-pic img{max-width:22px!important;max-height:22px!important}
    .pmd-icon-num{font-size:16px;font-weight:900;line-height:1.05}
    .pmd-icon-name{font-size:10px;color:#6b7280;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    ...
    .pmd-table th,.pmd-table td{padding:7px 8px;border-bottom:1px solid #eee;text-align:right}
  ...
  panel.innerHTML=`
    ...
    <div class="pmd-body">
      <div class="pmd-note">นับเฉพาะจุด Survey / Decision และจุดที่ผู้ใช้ปักในห้องปัจจุบัน ไม่รวมจุดคู่แข่ง</div>
  ...
  async function loadDash(){
    ...
    try{
      ...
      status.textContent=`${dashData.length.toLocaleString("th-TH")} จุด • จังหวัด ${knownP.toLocaleString("th-TH")} / อำเภอ ${knownD.toLocaleString("th-TH")} / ตำบล ${knownT.toLocaleString("th-TH")} • cache 5 นาที`;
    }catch(err){
    ...
    return `<div class="pmd-color-grid">${items.map(x=>`
      <div class="pmd-color-card">
        ...
        <span style="min-width:0">
          ...
          <div class="pmd-color-name">${esc(colorLabel(x.name))}</div>
        </span>
      </div>`).join("")}</div>`;
  }
  ...
  function exportDashboardExcel(){
    ...
    const statusRows=[
      ...
      ...ic.map(x=>[iconName(x.name),x.count,Number((x.count/rows.length*100).toFixed(2))])
    ];
    ...
    // Flat dashboard data: easiest sheet for Excel filter/sort/pivot.
    const rawRows=[[
      {v:"ลำดับ",s:1},{v:"จังหวัด",s:1},{v:"อำเภอ/เขต",s:1},{v:"ตำบล/แขวง",s:1},
      {v:"Area",s:1},{v:"จุดที่",s:1},{v:"Status",s:1},{v:"สี",s:1},{v:"HEX",s:1},
      {v:"Latitude",s:1},{v:"Longitude",s:1}
    ]];
    rows.forEach((r,i)=>{
      const hex=pinColor(r.color);
      rawRows.push([
        i+1,r.province,r.district,r.tambon,r.area||"ยังไม่ได้ระบุ",r.point_no||"",
        iconName(r._dashIcon),{v:pmColorDot(hex),s:pmColorStyle(hex)},hex,
    ...
    const sheets=[
      ...
      {name:"ข้อมูล Dashboard",rows:rawRows,options:{widths:[8,20,22,22,18,10,18,18,12,15,15]}},
  ...
  function renderDash(){
    ...
    root.innerHTML=`
      ...
      <div class="pmd-icon-grid">
        ${ic.map(x=>`<button type="button" class="pmd-icon-card ${dashIcon===x.name?"active":""}" data-icon="${esc(x.name)}"><span class="pmd-icon-pic">${iconVisual(x.name)}</span><span style="min-width:0"><div class="pmd-icon-num">${x.count.toLocaleString("th-TH")}</div><div class="pmd-icon-name">${esc(iconName(x.name))}</div></span></button>`).join("")||`<div class="pmd-empty">ไม่มีจุด</div>`}
      </div>
  ...
  try {
    ...
    let html = `<!doctype html><html><head><meta charset="UTF-8">
      <style>
        ...
        th,td{border:1px solid #cbd5e1;padding:6px 8px;font-size:11px;vertical-align:middle}
        th{background:#111827;color:#fff;font-weight:700;white-space:nowrap}
        .num{text-align:right}
        .center{text-align:center}
        .colorCell{font-weight:700;text-align:center;min-width:68px}
    ...
    allPins.forEach(p => {
      ...
      const cname = colorName(clr);
      // Choose readable text color for darker fills.
      const darkText = ["#eab308","#22c55e","#14b8a6","#f97316","#ec4899"].includes(clr.toLowerCase());
      const fg = darkText ? "#111111" : "#ffffff";

      html += `<tr>
        <td>${escExcel(p.title)}</td>
        <td class="center">${escExcel(p.point_no || "")}</td>
        <td>${escExcel(pinStatusName(p.icon))}</td>
        <td>${escExcel(pinStatusName(p.icon))}</td>
        <td class="colorCell" style="background:${escExcel(clr)};color:${fg}">${escExcel(cname)}</td>
        <td>${escExcel(p.address || "")}</td>
        <td>${escExcel(p.area || "")}</td>
        <td>${escExcel(p.note || "")}</td>
        <td class="num">${escExcel(p.lat)}</td>
        <td class="num">${escExcel(p.lng)}</td>
        <td>${escExcel(p.created_by || "")}</td>
        <td>${escExcel(p.created_at ? new Date(p.created_at).toLocaleString("th-TH") : "")}</td>
      </tr>`;
    });
    ...
    a.href = URL.createObjectURL(blob);
    a.download = `pinmap-${safeRoom}.xls`;
...
(function setupRequestedAdditions(){
  ...
  if (iconRow) {
    [
      ["decision", "Decision", "Decision.png"],
      ["van", "Van", "van.webp"],
      ["m1", "M1", "m1.png"],
      ["x", "X", "x.jpg"],
      ["deal", "Deal", "deal.png"]
    ].forEach(([key,title,src]) => {
      if (iconRow.querySelector(`[data-icon="${key}"]`)) return;
      const b = document.createElement("button");
      b.type = "button";
