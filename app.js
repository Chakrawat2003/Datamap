...
// Use ArcGIS REST tiles instead of the WMS endpoint. The WMS endpoint
// was returning failed tile requests on the deployed site.
const DOL_REST_URL = "https://landsgis.dol.go.th/arcgis/rest/services/DOL_PARCEL/MapServer";
const dolLayer = L.tileLayer(`${DOL_REST_URL}/tile/{z}/{y}/{x}`, {
  minZoom: 0,
  maxZoom: 20,
  maxNativeZoom: 19,
  tileSize: 256,
...
let dolTileErrors = 0;
let dolLoadTimer = null;

dolLayer.on("tileload", () => {
  dolTileLoaded = true;
  dolTileErrors = 0;
  if (dolLoadTimer) {
    clearTimeout(dolLoadTimer);
    dolLoadTimer = null;
  }
  if (dolOn) toast("เปิดชั้นข้อมูลแปลงที่ดิน (DOL) แล้ว");
});

dolLayer.on("tileerror", () => {
  if (!dolOn) return;
  dolTileErrors++;
  if (!dolTileLoaded && dolTileErrors >= 3 && !dolLoadTimer) {
    toast("DOL เปิดแล้ว แต่เซิร์ฟเวอร์ไม่ส่งข้อมูลแปลงที่ดิน");
  }
});
...
function toggleDol() {
  ...
  if (dolOn) {
    dolTileLoaded = false;
    dolTileErrors = 0;
    if (dolLoadTimer) clearTimeout(dolLoadTimer);

    dolLayer.addTo(map);
    toast("กำลังโหลดชั้นข้อมูลแปลงที่ดิน (DOL)...");

    dolLoadTimer = setTimeout(() => {
      dolLoadTimer = null;
      if (dolOn && !dolTileLoaded && dolTileErrors > 0) {
        toast("ไม่สามารถโหลดข้อมูล DOL ได้ — เซิร์ฟเวอร์กรมที่ดินไม่ตอบสนอง");
      }
    }, 7000);
  } else {
