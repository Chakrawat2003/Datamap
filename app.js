...
(function setupPinmapDashboard(){
  ...
  const DASH_ICON_NAMES = {
    pin:"Pin",star:"Star",heart:"Heart",flag:"Flag",home:"Home",food:"Food",
    shop:"Shop",hotel:"Hotel",photo:"Photo",van:"Van",m1:"M1",x:"X",deal:"Deal"
  };
  ...
  st.textContent=`
    ...
    @media(max-width:767px){
      #pinmapDashboardPanel{top:76px;right:7px;bottom:8px;width:calc(100% - 14px)!important;min-width:0;max-width:none;border-radius:14px}
  ...
  panel.innerHTML=`
    ...
    <div class="pmd-body">
      <div class="pmd-note">นับเฉพาะ Pins ที่ผู้ใช้ปักในห้องปัจจุบัน และตัด 7‑Eleven, CJ MORE, Tops, ถูกดี, Big C, Lawson108, Lotus และ Makro ออก</div>
  ...
  function locate(p){
    ...
    // Tambon polygon supplies tambon and normally amphoe/district attributes.
  ...
  async function loadDash(){
    ...
    root.innerHTML="";
    try{
      const fresh=dashRoom===S.room&&(Date.now()-dashLoadedAt)<60000;
      if(!fresh){
        dashRows=await fetchAllPinsRoom();
      ...
      status.textContent=`${dashData.length.toLocaleString("th-TH")} จุด • จับพื้นที่ได้ จังหวัด ${knownP.toLocaleString("th-TH")} / อำเภอ ${knownD.toLocaleString("th-TH")} / ตำบล ${knownT.toLocaleString("th-TH")} จุด`;
    }catch(err){
