...
(function setupPinmapRulerOnly() {
  ...
  function positionButton() {
    ...
    if (isMobile()) {
      // Keep the ruler at the same vertical level as the mobile search box.
      const search = document.getElementById("mSearchInput");
      const searchRect = search?.getBoundingClientRect?.();
      let top = 14;
      if (searchRect && searchRect.width > 0 && searchRect.height > 0) {
        top = Math.max(8, searchRect.top - mapRect.top + (searchRect.height - btn.offsetHeight) / 2);
      }
      btn.style.left = "12px";
...
(function setupRequestedAdditions(){
  ...
  roadCanvas.style.cssText = "position:absolute;left:0;top:0;pointer-events:none;z-index:400;display:none;";
  ...
  async function ensureRoadManifest(){
    ...
    roadManifestPromise=(async()=>{
      await pinmapLoadRoadScript("roads/manifest.js","pinmapRoadManifestScript");
      roadManifest=window.PINMAP_ROAD_MANIFEST;
  ...
  async function ensureRoadChunk(key){
    ...
    if(!window.PINMAP_ROAD_BUNDLES[bundleKey]){
      ...
      await pinmapLoadRoadScript(`roads/bundle_${bundleKey}.js`,`pinmapRoadBundle_${safe}`);
    }
    return window.PINMAP_ROAD_CHUNKS[key]||null;
  }
  ...
  async function redrawRoadCanvas(){
    ...
    // Tertiary + Residential: SAME yellow and SAME 2.0 px width.
    if(ROAD_GROUPS.local.on){
      for(const data of chunks){
        if(!data) continue;
        for(const s of (data.t||[])) drawEncodedRoad(s,"#ffd400",2.0,.96);
        for(const s of (data.r||[])) drawEncodedRoad(s,"#ffd400",2.0,.96);
      }
    }
  ...
  async function toggleRoadGroup(name){
    ...
    try{
      ...
      group.on=false;
      syncRoadButtons();
      toast("โหลดข้อมูลถนนไม่สำเร็จ — ตรวจสอบโฟลเดอร์ roads");
    }
  }
...
function pinmapApplyPointCap() {
  ...
  // Search mode intentionally ignores viewport and the 200-point render cap.
  // Normal mode remains viewport-only and capped at 200.
  ...
  let selectedPins, selectedS11, selectedCJ;

  if (searching) {
    selectedPins = pinList;
    selectedS11 = s11List;
    selectedCJ = cjList;
  } else {
    selectedPins = pinList.slice(0, max);
    let remaining = Math.max(0, max - selectedPins.length);
    selectedS11 = s11List.slice(0, remaining);
    remaining -= selectedS11.length;
    selectedCJ = cjList.slice(0, remaining);
  }
