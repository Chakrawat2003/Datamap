...
// Keep every point marker above road/boundary canvases.
// This affects marker display order only; it does not change data or Supabase reads.
...
(function setupPinmapAllPointsEye(){
  ...
  function positionEye(){
    ...
    if (isMobile()) {
      ...
      // Desktop: place the round eye immediately beside the ruler on the right.
      const ruler = document.getElementById("pinmapRulerBtn");
      const r = ruler?.getBoundingClientRect?.();
      if (r && r.width > 0 && r.height > 0) {
        btn.style.left = "auto";
        btn.style.right = `${Math.round(mapRect.right - r.left + 8)}px`;
        btn.style.top = `${Math.round(r.top - mapRect.top + (r.height - btn.offsetHeight) / 2)}px`;
        btn.style.bottom = "auto";
      } else {
        btn.style.left = "auto";
...
(function setupRequestedAdditions(){
  ...
  roadCanvas.style.cssText = "position:absolute;left:0;top:0;pointer-events:none;z-index:300;display:none;";
    ...
    if(ROAD_GROUPS.local.on){
      for(const data of chunks){
        ...
        for(const s of (data.r||[])) drawEncodedRoad(s,"#ffd400",2.0,.96);
          for(const s of (data.u||[])) drawEncodedRoad(s,"#ffd400",2.0,.96);
      }
    }
