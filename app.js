...
let PINMAP_DASHBOARD_FILTER_ACTIVE = false;
let PINMAP_DASHBOARD_FILTER_PIN_IDS = null;
function pinmapSetDashboardIconMapFilter(icon){
  PINMAP_DASHBOARD_ICON_FILTER = String(icon || "").trim().toLowerCase();
}
function pinmapSetDashboardCombinedMapFilter(rows, active){
  PINMAP_DASHBOARD_FILTER_ACTIVE = !!active;
  PINMAP_DASHBOARD_FILTER_PIN_IDS = PINMAP_DASHBOARD_FILTER_ACTIVE
    ? new Set((rows || []).map(p => String(p.id)))
    : null;
...
function pinmapBuildCompetitorPanel(){
  const build=(panel,mobile=false)=>{if(!panel)return;const oldExtraOpen=!!panel.querySelector(".pinmap-comp-extra:not(.hidden)");
    const lotusDef=PINMAP_EXTRA_COMPETITOR_DEFS.find(d=>d.key==="lotus");
    const lotusSt=lotusDef?PINMAP_EXTRA_COMPETITORS.get("lotus"):null;
    const lotusRow=lotusDef&&lotusSt?pinmapCompRow(`${mobile?"m":""}pc_lotus`,`${mobile?"m":""}pcadd_lotus`,lotusDef.label,lotusDef.logo,lotusSt.on,lotusSt.rows.length?String(lotusSt.rows.length):""):"";
    const otherRows=PINMAP_EXTRA_COMPETITOR_DEFS.filter(d=>d.key!=="lotus").map(d=>{const st=PINMAP_EXTRA_COMPETITORS.get(d.key);return pinmapCompRow(`${mobile?"m":""}pc_${d.key}`,`${mobile?"m":""}pcadd_${d.key}`,d.label,d.logo,st.on,st.rows.length?String(st.rows.length):"")}).join("");
    panel.innerHTML=`<div class="pinmap-comp-panel">${pinmapCompRow(mobile?"mS11Toggle":"s11Toggle",mobile?"mS11Add":"s11Add","7-Eleven","7-11-logo.png",S11.on)}${pinmapCompRow(mobile?"mCJToggle":"cjToggle",mobile?"mCJAdd":"cjAdd","CJ MORE","logo-cj-more.jpg",CJ.on)}${lotusRow}<button type="button" class="pinmap-comp-more">อื่นๆ ▾</button><div class="pinmap-comp-extra ${oldExtraOpen?"":"hidden"}">${otherRows}</div></div>`;
...
if ($("save")) {
  $("save").onclick = async () => {
    ...
    $("save").disabled = true;
    try {
      // Local expiry check; refresh Auth only if JWT is inside the safety window.
      // This does not reload Pins or add a database read.
      await pinmapEnsureFreshSession(false);
      let savedRow=null;
      if (S.editing) {
...
if ($("delete")) {
  $("delete").onclick = async () => {
    ...
    try {
      // Keep delete from racing JWT expiry. No Pins reload is performed.
      await pinmapEnsureFreshSession(false);
...
(function setupPinmapDashboard(){
  ...
  let dashRows=[], dashData=[], dashRoom="", dashLoadedAt=0, dashCacheStale=false;
  // Dashboard cache lasts for the whole current room/session.
  // Realtime deltas keep it current. A full reload is required only when:
  // 1) opening Dashboard for the first time,
  // 2) changing room, or
  // 3) Realtime reports a disconnect/error and marks the cache stale.
  ...
  panel.innerHTML=`
    ...
    <div class="pmd-body">
      ...
      <div class="pmd-filters">
        ...
        <button id="pmdClearFilters" type="button" class="pmd-select" style="cursor:pointer;font-weight:800;background:#111;color:#fff;border-color:#111">Clear การกรองทั้งหมด</button>
      </div>
  ...
  $("pmdClearFilters").onclick=()=>{
    ...
    ["pmdProvince","pmdDistrict","pmdTambon","pmdIcon","pmdArea","pmdPointNo","pmdPinner"].forEach(id=>{
      const el=$(id);
      if(el) el.value="";
    });
    pinmapSetDashboardCombinedMapFilter([], false);
    renderDash();
  };
  ...
  function dashboardRebuildFromCache(renderIfOpen=true){
    ...
    if(status && !panel.classList.contains("hidden")){
      ...
      status.textContent=`${dashData.length.toLocaleString("th-TH")} จุด • จังหวัด ${knownP.toLocaleString("th-TH")} / อำเภอ ${knownD.toLocaleString("th-TH")} / ตำบล ${knownT.toLocaleString("th-TH")} • Realtime + cache ตลอด Session`;
    }
  }

  window.pinmapDashboardMarkStale=function(){
    // Do not fetch immediately. Keep the existing Dashboard/map data visible.
    // The next Dashboard open will reconcile once from Supabase.
    if(dashRoom===S.room && dashLoadedAt) dashCacheStale=true;
  };
  ...
  window.pinmapDashboardApplyPinDelta=function(payload){
    try{
      ...
      // Receiving valid Realtime deltas keeps the session cache current.
  ...
  async function loadDash(){
    ...
    try{
      ...
      const reusable=dashRoom===S.room && !!dashLoadedAt && !dashCacheStale;
      if(!reusable){
        dashRows=await fetchDashboardPinsRoom();
        dashRoom=S.room;
        dashLoadedAt=Date.now();
        dashCacheStale=false;
        geoCache.clear();
      }
      dashData=dashRows.filter(p=>!excluded(p)).map(p=>({...p,...locate(p),_dashIcon:ni(p.icon)}));
      populate();renderDash();
  ...
  function renderDash(){
    ...
    const dashboardMapFilterActive = !!(dashProvince || dashDistrict || dashTambon || dashIcon || dashArea || dashPointNo || dashPinner);
    pinmapSetDashboardCombinedMapFilter(rows, dashboardMapFilterActive);
...
async function pinmapBackfillCurrentRoomAddresses() {
  ...
  try {
    // Egress: do not fetch every Pin in the room again just for Address backfill.
    const rows = Array.isArray(S.all) ? S.all : [];
...
async function loadPinsViewport() {
  ...
  // Usually returns immediately from a local expiry check; no extra Pins query.
  await pinmapEnsureFreshSession(false);
  ...
  // If a request races exactly with JWT expiry, refresh once and retry once.
  // Never replace the existing Pin cache with an auth-error result.
  if (error && pinmapLooksLikeJwtExpired(error)) {
    await pinmapEnsureFreshSession(true);
  ...
  if (error) {
    // Never blank the map because a refresh/network request failed.
    console.warn("loadPinsViewport:", error);
    if (!pinmapLooksLikeJwtExpired(error)) toast(error.message);
    return;
  }
...
// First-entry fallback for a newly approved member/device.
// Normal room entry still loads only the current viewport. If that returns 0 rows,
// fetch at most the newest visible-cap rows from the room once, then move the map there.
// This avoids loading the whole room and keeps Point Cap unchanged.
async function pinmapLoadInitialRoomPinsFallback() {
  if (!S.sb || !S.room || (Array.isArray(S.all) && S.all.length)) return false;
  ...
  if (error) {
    console.warn("initial room pins fallback:", error);
    return false;
  }
  ...
  if (pts.length) {
    try {
      ...
      if (b.isValid()) {
        map.fitBounds(b.pad(0.10), { maxZoom: 16, animate: false });
      }
    } catch (e) {
      console.warn("initial room pins fit:", e);
    }
  }
  return true;
}
  ...
  S.ch = S.sb.channel(`pins-${S.room}`, {
    ...
    .subscribe(async status => {
      ...
      if (status === "SUBSCRIBED") {
        ...
        // Recover the websocket in the background. This performs no Pins query.
        if (status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED") {
          // Dashboard keeps showing its current cache. Mark it stale so the NEXT
          // Dashboard open reconciles once; do not download all Pins immediately.
          if (typeof window.pinmapDashboardMarkStale === "function") {
            window.pinmapDashboardMarkStale();
          }
          const roomAtFailure = S.room;
          setTimeout(async () => {
            if (!S.sb || !S.session || !roomAtFailure || S.room !== roomAtFailure) return;
            await pinmapEnsureFreshSession(false);
            if (S.room === roomAtFailure) subscribeRealtime();
          }, 1500);
        }
      }
    });
...
async function enterRoom(room) {
  ...
  await loadPins();
  await pinmapLoadInitialRoomPinsFallback();
...
function pinmapApplyPointCap() {
  ...
  const pinList = S.all.filter(p =>
    ...
    (!PINMAP_DASHBOARD_FILTER_ACTIVE ||
      (PINMAP_DASHBOARD_FILTER_PIN_IDS && PINMAP_DASHBOARD_FILTER_PIN_IDS.has(String(p.id))))
  );
...
let pinmapAuthRefreshBusy = false;
let pinmapAuthRefreshPromise = null;
let pinmapLastAuthRefreshAt = 0;
let pinmapLastAuthRefreshFailureAt = 0;
const PINMAP_AUTH_REFRESH_AHEAD_SEC = 10 * 60;
const PINMAP_AUTH_LOCAL_CHECK_MS = 2 * 60 * 1000;

function pinmapSessionExpiresSoon(session, seconds = 300) {
  const exp = Number(session?.expires_at || 0);
  if (!exp) return false;
  return (exp * 1000 - Date.now()) <= seconds * 1000;
}

async function pinmapApplyFreshAccessToken(session, reconnectRoomRealtime = true) {
  if (!S.sb || !session?.access_token) return;

  // Supabase Realtime can otherwise keep using the JWT that existed when
  // the websocket/channel was first created.
  try {
    if (S.sb.realtime && typeof S.sb.realtime.setAuth === "function") {
      await S.sb.realtime.setAuth(session.access_token);
    }
  } catch (e) {
    console.warn("Realtime JWT update:", e);
  }

  // Recreate only the current room's realtime channel. This does NOT query Pins.
  // It only restores websocket/presence with the fresh token.
  if (reconnectRoomRealtime && S.room) {
    try { subscribeRealtime(); } catch (e) { console.warn("Room realtime reconnect:", e); }
  }
}

async function pinmapEnsureFreshSession(force = false) {
  if (!S.sb) return S.session;

  // Collapse simultaneous callers into one refresh request.
  if (pinmapAuthRefreshPromise) return pinmapAuthRefreshPromise;

  // Pure local check: no Supabase request unless token is close to expiry.
  if (!force && S.session && !pinmapSessionExpiresSoon(S.session, PINMAP_AUTH_REFRESH_AHEAD_SEC)) {
    return S.session;
  }

  pinmapAuthRefreshBusy = true;
  pinmapAuthRefreshPromise = (async () => {
    try {
      ...
      if (data?.session) {
        S.session = data.session;
        pinmapLastAuthRefreshAt = Date.now();
        pinmapLastAuthRefreshFailureAt = 0;
        await pinmapApplyFreshAccessToken(data.session, true);
        refreshUserUI();
        return data.session;
      }
    } catch (e) {
      pinmapLastAuthRefreshFailureAt = Date.now();
      console.warn("PinMap session refresh failed:", e);
      // Critical UX rule:
      // temporary auth/network failures must NOT clear room or pins already on screen.
      return S.session;
    } finally {
      pinmapAuthRefreshBusy = false;
      pinmapAuthRefreshPromise = null;
    }
    return S.session;
  })();

  return pinmapAuthRefreshPromise;
}

function pinmapLooksLikeJwtExpired(error) {
...
// Browser background tabs may throttle timers. Re-check the token whenever
// the user returns to the page, plus a very light periodic check.
window.addEventListener("online", () => {
  if (!S.session) return;
  void (async () => {
    await pinmapEnsureFreshSession(false);
    if (S.room) subscribeRealtime();
  })();
});

document.addEventListener("visibilitychange", () => {
  if (!document.hidden && S.session) void pinmapEnsureFreshSession(false);
});
window.addEventListener("focus", () => {
  if (S.session) void pinmapEnsureFreshSession(false);
});

// Local clock check only. No Pins query and no database read.
// Refresh is sent only when the JWT is already within the 10-minute safety window.
setInterval(() => {
  if (S.session && pinmapSessionExpiresSoon(S.session, PINMAP_AUTH_REFRESH_AHEAD_SEC)) {
    void pinmapEnsureFreshSession(false);
  }
}, PINMAP_AUTH_LOCAL_CHECK_MS);
    ...
    try {
      ...
      S.sb.auth.onAuthStateChange((event, session) => {
        ...
        if (event === "TOKEN_REFRESHED" && session) {
          // Keep current room + current Pin cache intact.
          // Update the websocket JWT in place; no Pins reload and no saved-room reload.
          void pinmapApplyFreshAccessToken(session, false);
          refreshUserUI();
          return;
        }

        if (event === "SIGNED_OUT") {
          // Auth is genuinely gone. UI may lock, but do not erase room/pin state here.
          onAuthReady();
          return;
        }
