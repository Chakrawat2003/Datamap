...
/* =========================================================
   PINMAP DASHBOARD
   Counts only user Pins and excludes competitor-logo pins.
   Province / district are derived locally from loaded boundary GeoJSON.
   ========================================================= */
(function setupPinmapDashboard(){
  ...
  const DASH_ICON_NAMES={pin:"Pin",star:"Star",heart:"Heart",flag:"Flag",home:"Home",food:"Food",shop:"Shop",hotel:"Hotel",photo:"Photo",van:"Van",m1:"M1",x:"X",deal:"Deal"};
  let dashRows=[], dashData=[], dashRoom="", dashLoadedAt=0;
  let dashProvince="", dashDistrict="", dashIcon="";
  ...
  st.textContent=`
    ...
    #pinmapDashboardPanel{position:absolute;z-index:1450;right:14px;top:86px;bottom:16px;width:520px;min-width:340px;max-width:min(76vw,900px);background:rgba(255,255,255,.985);border:1px solid #ddd;border-radius:16px;box-shadow:0 10px 35px rgba(0,0,0,.22);overflow:hidden;display:flex;flex-direction:column;font-family:inherit}
    #pinmapDashboardPanel.hidden{display:none!important}.pmd-resize{position:absolute;left:-5px;top:0;bottom:0;width:11px;cursor:ew-resize;touch-action:none;z-index:4}
    .pmd-head{flex:0 0 auto;padding:14px 15px 11px;border-bottom:1px solid #e7e7e7;display:flex;align-items:center;gap:10px;background:#fff}.pmd-title{font-size:17px;font-weight:800;flex:1}.pmd-close{width:34px;height:34px;border:0;border-radius:9px;background:#f1f1f1;font-size:20px;cursor:pointer}
    .pmd-body{overflow:auto;padding:13px 14px 18px}.pmd-note{font-size:11px;color:#6b7280;line-height:1.45;margin:0 0 10px}.pmd-filters{display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px;margin-bottom:12px}.pmd-select{width:100%;min-width:0;padding:8px 9px;border:1px solid #d1d5db;border-radius:9px;background:#fff;font:inherit;font-size:12px}
    .pmd-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:13px}.pmd-kpi{background:#f6f7f8;border-radius:11px;padding:10px 9px;min-width:0}.pmd-kpi-v{font-size:20px;font-weight:900;line-height:1.15}.pmd-kpi-l{font-size:10px;color:#6b7280;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .pmd-section{margin:13px 0 5px;font-weight:800;font-size:13px}.pmd-icon-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(105px,1fr));gap:7px}.pmd-icon-card{border:1px solid #e5e7eb;background:#fff;border-radius:11px;padding:9px;display:flex;align-items:center;gap:8px;cursor:pointer;min-width:0;text-align:left}.pmd-icon-card.active{outline:2px solid #111}
    .pmd-icon-pic{width:31px;height:31px;border-radius:8px;background:#f3f4f6;display:flex;align-items:center;justify-content:center;overflow:hidden;flex:0 0 auto;font-size:19px}.pmd-icon-pic img{width:26px;height:26px;object-fit:contain;display:block}.pmd-icon-num{font-size:16px;font-weight:900;line-height:1.05}.pmd-icon-name{font-size:10px;color:#6b7280;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .pmd-chart-grid{display:grid;grid-template-columns:1fr 1.25fr;gap:10px}.pmd-card{border:1px solid #e5e7eb;border-radius:12px;background:#fff;padding:11px;min-width:0}.pmd-card-title{font-weight:800;font-size:12px;margin-bottom:8px}.pmd-donut-wrap{display:flex;gap:11px;align-items:center}.pmd-donut{width:112px;height:112px;border-radius:50%;position:relative;flex:0 0 auto}.pmd-donut:after{content:"";position:absolute;inset:27px;background:#fff;border-radius:50%}.pmd-legend{min-width:0;flex:1;font-size:10px;line-height:1.55}.pmd-legend-row{display:flex;justify-content:space-between;gap:8px;border-bottom:1px dotted #eee}
    .pmd-bars{display:flex;flex-direction:column;gap:7px}.pmd-bar-row{display:grid;grid-template-columns:95px 1fr 34px;gap:6px;align-items:center;font-size:10px}.pmd-bar-name{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pmd-bar-track{height:8px;background:#eee;border-radius:999px;overflow:hidden}.pmd-bar-fill{height:100%;background:#111;border-radius:999px}.pmd-bar-val{text-align:right;font-weight:800}
    .pmd-table-wrap{overflow:auto;border:1px solid #e5e7eb;border-radius:11px}.pmd-table{border-collapse:collapse;width:100%;font-size:10px;white-space:nowrap}.pmd-table th,.pmd-table td{padding:7px 8px;border-bottom:1px solid #eee;text-align:right}.pmd-table th:first-child,.pmd-table td:first-child{text-align:left;position:sticky;left:0;background:#fff}.pmd-table th{background:#f8f8f8;font-weight:800;position:sticky;top:0;z-index:1}.pmd-empty{padding:30px 10px;text-align:center;color:#777;font-size:12px}.pmd-status{font-size:11px;color:#666;margin:6px 0 10px}
    @media(max-width:767px){#pinmapDashboardPanel{top:76px;right:7px;bottom:8px;width:calc(100% - 14px)!important;min-width:0;max-width:none;border-radius:14px}.pmd-resize{display:none}.pmd-filters{grid-template-columns:1fr}.pmd-kpis{grid-template-columns:1fr 1fr}.pmd-chart-grid{grid-template-columns:1fr}}
  `;
  document.head.appendChild(st);

  function makeBtn(baseId,id,mobile){const base=$(baseId);if(!base||$(id))return;const b=base.cloneNode(false);b.id=id;b.classList.add("pinmap-dashboard-btn");b.innerHTML="📊 Dashboard";base.insertAdjacentElement("afterend",b);b.onclick=()=>{if(mobile&&typeof closeMobileMenu==="function")closeMobileMenu();toggleDash()}}
  makeBtn("tableToggle","dashboardToggle",false);makeBtn("mTableToggle","mDashboardToggle",true);

  const panel=document.createElement("section");panel.id="pinmapDashboardPanel";panel.className="hidden";panel.innerHTML=`<div class="pmd-resize" id="pmdResize"></div><div class="pmd-head"><div class="pmd-title">📊 Dashboard จุดที่ปัก</div><button class="pmd-close" id="pmdClose" type="button">×</button></div><div class="pmd-body"><div class="pmd-note">นับเฉพาะ Pins ที่ผู้ใช้ปัก และตัด 7‑Eleven, CJ MORE, Tops, ถูกดี, Big C, Lawson108, Lotus และ Makro ออก</div><div class="pmd-filters"><select id="pmdProvince" class="pmd-select"><option value="">ทุกจังหวัด</option></select><select id="pmdDistrict" class="pmd-select"><option value="">ทุกอำเภอ/เขต</option></select><select id="pmdIcon" class="pmd-select"><option value="">ทุกประเภทหมุด</option></select></div><div id="pmdStatus" class="pmd-status"></div><div id="pmdContent"></div></div>`;
  map.getContainer().appendChild(panel);L.DomEvent.disableClickPropagation(panel);L.DomEvent.disableScrollPropagation(panel);
  $("pmdClose").onclick=()=>setOpen(false);
  $("pmdProvince").onchange=e=>{dashProvince=e.target.value;dashDistrict="";refreshDistricts();renderDash()};
  $("pmdDistrict").onchange=e=>{dashDistrict=e.target.value;renderDash()};
  $("pmdIcon").onchange=e=>{dashIcon=e.target.value;renderDash()};

  const rh=$("pmdResize");rh.addEventListener("pointerdown",e=>{if(matchMedia("(max-width:767px)").matches)return;e.preventDefault();rh.setPointerCapture(e.pointerId);const sx=e.clientX,sw=panel.getBoundingClientRect().width;const mv=ev=>{const mw=Math.min(innerWidth*.76,900),nw=Math.max(340,Math.min(mw,sw+(sx-ev.clientX)));panel.style.width=Math.round(nw)+"px"};const up=ev=>{rh.releasePointerCapture?.(ev.pointerId);rh.removeEventListener("pointermove",mv);rh.removeEventListener("pointerup",up);rh.removeEventListener("pointercancel",up)};rh.addEventListener("pointermove",mv);rh.addEventListener("pointerup",up);rh.addEventListener("pointercancel",up)});
  function setOpen(on){panel.classList.toggle("hidden",!on);$("dashboardToggle")?.classList.toggle("active",on);$("mDashboardToggle")?.classList.toggle("active",on)}
  async function toggleDash(){if(!S.room)return toast("กรุณาเข้าห้องก่อน");if(!panel.classList.contains("hidden")){setOpen(false);return}setOpen(true);await loadDash()}

  const ni=v=>String(v||"pin").trim().toLowerCase();const excluded=p=>DASH_EXCLUDED_ICONS.has(ni(p?.icon));
  function iconVisual(i){const k=ni(i);if(typeof PIN_LOGOS!=="undefined"&&PIN_LOGOS[k])return `<img src="${esc(PIN_LOGOS[k])}" alt="">`;if(typeof ICONS!=="undefined"&&ICONS[k])return esc(ICONS[k]);return "📍"}
  const iconName=i=>DASH_ICON_NAMES[ni(i)]||ni(i)||"Pin";
  function first(p,ks){for(const k of ks){const v=p?.[k];if(v!==undefined&&v!==null&&String(v).trim())return String(v).trim()}return""}
  function inRing(x,y,r){let inside=false;if(!Array.isArray(r))return false;for(let i=0,j=r.length-1;i<r.length;j=i++){const a=r[i],b=r[j];if(!a||!b)continue;const xi=+a[0],yi=+a[1],xj=+b[0],yj=+b[1];if(((yi>y)!==(yj>y))&&(x<(xj-xi)*(y-yi)/((yj-yi)||1e-12)+xi))inside=!inside}return inside}
  function inPoly(x,y,p){if(!p?.length||!inRing(x,y,p[0]))return false;for(let i=1;i<p.length;i++)if(inRing(x,y,p[i]))return false;return true}
  function inGeom(x,y,g){if(!g)return false;if(g.type==="Polygon")return inPoly(x,y,g.coordinates);if(g.type==="MultiPolygon")return (g.coordinates||[]).some(p=>inPoly(x,y,p));return false}
  function locate(p){const key=`${(+p.lat).toFixed(6)},${(+p.lng).toFixed(6)}`;if(geoCache.has(key))return geoCache.get(key);const y=+p.lat,x=+p.lng;let province="",district="";for(const f of window.PINMAP_TAMBON_BOUNDARIES?.features||[]){if(!inGeom(x,y,f.geometry))continue;const q=f.properties||{};province=first(q,["PROV_NAMT","PROV_NAME","PROVINCE","CHANGWAT","ADM1_TH","NAME_1","P_NAME_T"]);district=first(q,["AMP_NAMT","AMPHOE_T","AMPHOE","A_NAME_T","DISTRICT","DISTRICT_T","ADM2_TH","NAME_2","AMP_NAME","AMP_NAME_T"]);break}if(!province){for(const f of window.PINMAP_PROVINCE_BOUNDARIES?.features||[]){if(!inGeom(x,y,f.geometry))continue;province=first(f.properties||{},["PROV_NAMT","PROV_NAME","PROVINCE","CHANGWAT","ADM1_TH","NAME_1","P_NAME_T","name"]);break}}const v={province:province||"ไม่ทราบจังหวัด",district:district||"ไม่ทราบอำเภอ/เขต"};geoCache.set(key,v);return v}
  const uniq=a=>[...new Set(a.filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),"th"));
  function opts(sel,vals,label,current=""){sel.innerHTML=`<option value="">${label}</option>`+vals.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join("");sel.value=vals.includes(current)?current:""}
  function refreshDistricts(){const pool=dashProvince?dashData.filter(x=>x.province===dashProvince):dashData;const vals=uniq(pool.map(x=>x.district));opts($("pmdDistrict"),vals,"ทุกอำเภอ/เขต",dashDistrict);dashDistrict=$("pmdDistrict").value||""}
  function populate(){opts($("pmdProvince"),uniq(dashData.map(x=>x.province)),"ทุกจังหวัด",dashProvince);opts($("pmdIcon"),uniq(dashData.map(x=>x._dashIcon)),"ทุกประเภทหมุด",dashIcon);refreshDistricts()}
  async function loadDash(){const status=$("pmdStatus"),root=$("pmdContent");status.textContent="กำลังเตรียม Dashboard...";root.innerHTML="";try{const fresh=dashRoom===S.room&&(Date.now()-dashLoadedAt)<60000;if(!fresh){dashRows=await fetchAllPinsRoom();dashRoom=S.room;dashLoadedAt=Date.now()}dashData=dashRows.filter(p=>!excluded(p)).map(p=>({...p,...locate(p),_dashIcon:ni(p.icon)}));populate();renderDash();status.textContent=`${dashData.length.toLocaleString("th-TH")} จุด • เปลี่ยนตัวกรองได้โดยไม่ query เพิ่ม`}catch(err){console.error(err);status.textContent="";root.innerHTML=`<div class="pmd-empty">โหลด Dashboard ไม่สำเร็จ<br>${esc(err?.message||"")}</div>`}}
  function rowsNow(){return dashData.filter(x=>(!dashProvince||x.province===dashProvince)&&(!dashDistrict||x.district===dashDistrict)&&(!dashIcon||x._dashIcon===dashIcon))}
  function countBy(rows,fn){const m=new Map();rows.forEach(r=>{const k=fn(r)||"ไม่ทราบ";m.set(k,(m.get(k)||0)+1)});return [...m].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||String(a.name).localeCompare(String(b.name),"th"))}
  function donut(items,total){if(!total)return `<div class="pmd-empty">ไม่มีข้อมูล</div>`;const pal=["#111827","#2563eb","#16a34a","#f59e0b","#dc2626","#7c3aed","#0891b2","#db2777","#64748b","#84cc16"];let cur=0;const parts=items.map((v,i)=>{const a=cur/total*360;cur+=v.count;return `${pal[i%pal.length]} ${a}deg ${cur/total*360}deg`});return `<div class="pmd-donut-wrap"><div class="pmd-donut" style="background:conic-gradient(${parts.join(",")})"></div><div class="pmd-legend">${items.slice(0,8).map(v=>`<div class="pmd-legend-row"><span>${esc(iconName(v.name))}</span><b>${v.count.toLocaleString("th-TH")}</b></div>`).join("")}</div></div>`}
  function bars(items){const a=items.slice(0,10),mx=Math.max(1,...a.map(x=>x.count));if(!a.length)return `<div class="pmd-empty">ไม่มีข้อมูล</div>`;return `<div class="pmd-bars">${a.map(x=>`<div class="pmd-bar-row"><div class="pmd-bar-name" title="${esc(x.name)}">${esc(x.name)}</div><div class="pmd-bar-track"><div class="pmd-bar-fill" style="width:${Math.max(2,x.count/mx*100)}%"></div></div><div class="pmd-bar-val">${x.count.toLocaleString("th-TH")}</div></div>`).join("")}</div>`}
  function breakdown(rows,icons){const m=new Map();rows.forEach(r=>{const k=r.province+"|||"+r.district;if(!m.has(k))m.set(k,{province:r.province,district:r.district,total:0,icons:{}});const g=m.get(k);g.total++;g.icons[r._dashIcon]=(g.icons[r._dashIcon]||0)+1});const a=[...m.values()].sort((x,y)=>y.total-x.total);if(!a.length)return `<div class="pmd-empty">ไม่มีข้อมูลตามตัวกรอง</div>`;return `<div class="pmd-table-wrap"><table class="pmd-table"><thead><tr><th>จังหวัด / อำเภอ-เขต</th><th>รวม</th>${icons.map(i=>`<th>${iconVisual(i)} ${esc(iconName(i))}</th>`).join("")}</tr></thead><tbody>${a.map(g=>`<tr><td><b>${esc(g.province)}</b><br>${esc(g.district)}</td><td><b>${g.total.toLocaleString("th-TH")}</b></td>${icons.map(i=>`<td>${(g.icons[i]||0).toLocaleString("th-TH")}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`}
  function renderDash(){const root=$("pmdContent"),rows=rowsNow(),ic=countBy(rows,x=>x._dashIcon),pc=countBy(rows,x=>x.province),dc=countBy(rows,x=>x.district),icons=uniq(rows.map(x=>x._dashIcon));root.innerHTML=`<div class="pmd-kpis"><div class="pmd-kpi"><div class="pmd-kpi-v">${rows.length.toLocaleString("th-TH")}</div><div class="pmd-kpi-l">จุดที่ปักทั้งหมด</div></div><div class="pmd-kpi"><div class="pmd-kpi-v">${ic.length}</div><div class="pmd-kpi-l">ประเภทหมุด</div></div><div class="pmd-kpi"><div class="pmd-kpi-v" style="font-size:13px">${esc(pc[0]?.name||"—")}</div><div class="pmd-kpi-l">จังหวัดมากสุด</div></div><div class="pmd-kpi"><div class="pmd-kpi-v" style="font-size:13px">${esc(dc[0]?.name||"—")}</div><div class="pmd-kpi-l">อำเภอ/เขตมากสุด</div></div></div><div class="pmd-section">จำนวนตามรูปหมุด</div><div class="pmd-icon-grid">${ic.map(x=>`<button type="button" class="pmd-icon-card ${dashIcon===x.name?"active":""}" data-icon="${esc(x.name)}"><span class="pmd-icon-pic">${iconVisual(x.name)}</span><span style="min-width:0"><div class="pmd-icon-num">${x.count.toLocaleString("th-TH")}</div><div class="pmd-icon-name">${esc(iconName(x.name))}</div></span></button>`).join("")||`<div class="pmd-empty">ไม่มีจุด</div>`}</div><div class="pmd-section">ภาพรวมเพิ่มเติม</div><div class="pmd-chart-grid"><div class="pmd-card"><div class="pmd-card-title">สัดส่วนประเภทหมุด</div>${donut(ic,rows.length)}</div><div class="pmd-card"><div class="pmd-card-title">10 จังหวัดที่มีจุดปักมากที่สุด</div>${bars(pc)}</div></div><div class="pmd-section">10 อำเภอ/เขตที่มีจุดปักมากที่สุด</div><div class="pmd-card">${bars(dc)}</div><div class="pmd-section">สรุปจังหวัด → อำเภอ/เขต → ประเภทหมุด</div>${breakdown(rows,icons)}`;root.querySelectorAll(".pmd-icon-card").forEach(b=>b.onclick=()=>{dashIcon=dashIcon===b.dataset.icon?"":b.dataset.icon;$("pmdIcon").value=dashIcon;renderDash()})}
