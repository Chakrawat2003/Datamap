...
L.tileLayer(esriUrl, { maxZoom: 19, maxNativeZoom: 19, attribution: "Tiles © Esri" }).addTo(map);
...
/* ── PASSWORD RESET (OTP) ──
   Uses Supabase recovery OTP instead of a clickable recovery URL.
   This avoids email-security scanners (e.g. Outlook Safe Links) consuming
   the single-use recovery link before the user can open it.
*/
...
function openResetModal() {
  ...
  if ($("newPasswordWrap")) $("newPasswordWrap").classList.add("hidden");
  if ($("verifyResetOtp")) $("verifyResetOtp").disabled = false;
  if ($("saveNewPassword")) $("saveNewPassword").disabled = false;
  resetMsg("");
...
async function requestResetOtp() {
  ...
  const btn = $("verifyResetOtp");
  const resend = $("resendResetOtp");
  if (resend) resend.disabled = true;
  try {
    const redirectTo = window.location.origin + window.location.pathname;
    const { error } = await S.sb.auth.resetPasswordForEmail(email, { redirectTo });
    if (error) throw error;
    resetMsg("ส่งรหัส OTP ไปที่อีเมลแล้ว กรุณาตรวจสอบ Outlook แล้วกรอกรหัส 6 หลัก", true);
    if ($("resetHint")) $("resetHint").textContent = "กรอกรหัส OTP 6 หลักที่ส่งไปยังอีเมลของคุณ";
  } catch (err) {
    resetMsg(err.message || "ส่งรหัส OTP ไม่สำเร็จ");
  } finally {
    if (btn) btn.disabled = false;
    setTimeout(() => { if (resend) resend.disabled = false; }, 60000);
  }
}
...
async function verifyResetOtp() {
  ...
  const token = ($("resetOtp")?.value || "").replace(/\D/g, "");
  if (!email) return resetMsg("กรุณากรอกอีเมล");
  if (!/^\d{6}$/.test(token)) return resetMsg("กรุณากรอกรหัส OTP 6 หลัก");
  const btn = $("verifyResetOtp");
  if (btn) btn.disabled = true;
  try {
    const { data, error } = await S.sb.auth.verifyOtp({ email, token, type: "recovery" });
    if (error) throw error;
    if (!data?.session) throw new Error("ยืนยัน OTP สำเร็จ แต่ไม่พบเซสชันสำหรับเปลี่ยนรหัสผ่าน");
    ...
    resetMsg("ยืนยัน OTP สำเร็จแล้ว กรุณาตั้งรหัสผ่านใหม่", true);
    if ($("resetPassword")) $("resetPassword").focus();
  } catch (err) {
    resetMsg(err.message || "รหัส OTP ไม่ถูกต้องหรือหมดอายุ");
  } finally {
    if (btn) btn.disabled = false;
  }
}
...
async function saveNewPassword() {
  ...
  try {
    ...
    if (error) throw error;
    resetMsg("เปลี่ยนรหัสผ่านสำเร็จแล้ว กำลังเข้าสู่ระบบ...", true);
    setTimeout(() => {
...
if ($("verifyResetOtp")) $("verifyResetOtp").onclick = verifyResetOtp;
if ($("saveNewPassword")) $("saveNewPassword").onclick = saveNewPassword;
if ($("resendResetOtp")) $("resendResetOtp").onclick = requestResetOtp;
