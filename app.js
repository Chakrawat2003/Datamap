...
(function setupPinmapDashboard(){
  ...
  const DASH_PIN_SELECT = "id,lat,lng,area,icon,color,point_no,title,created_by";
  ...
  window.pinmapDashboardApplyPinDelta=function(payload){
    try{
      ...
      const slim={
        ...
        point_no:row.point_no,title:row.title,created_by:row.created_by
      };
  ...
  function exportDashboardExcel(){
    ...
    const sheets=[
      ...
      {name:"ข้อมูล Dashboard",rows:rawRows,options:{widths:[8,30,10,20,22,22,18,18,22,18,12,15,15]}},
...
/* Compact 7-logo selector — label stays outside/below the logo */
.iconRow{
...
@media(max-width:520px){
  ...
  .pinLogoLabel{font-size:9.5px !important}
}
    ...
    allPins.forEach(p => {
      ...
      html += `<tr>
        ...
        <td>${escExcel(pinDetailedStatus(p.icon,p.color))}</td>
