...
// Prevent browser/password-manager login suggestions from covering the map search boxes.
function pinmapDisableSearchAutofill() {
  ["search", "mSearchInput"].forEach(id => {
    ...
    // Use a non-login-like field name so Chrome/password managers do not treat it as username.
    el.setAttribute("name", `pinmap-place-search-${id}`);

    // Common password-manager ignore hints.
...
const handleSearch = () => {
  ...
  // 7-Eleven / CJ MORE are local static datasets, so their search is immediate.
  s11Update();
  cjUpdate();

  pinmapSearchTimer = setTimeout(() => {
    if (q) loadPinsSearch(q);
    else loadPinsViewport();
  }, 220);
};
...
function applyPinRealtimeDelta(payload) {
  ...
  // Normal mode remains capped at 300; search mode is intentionally uncapped.
...
function pinmapApplyPointCap() {
  ...
  // Search mode intentionally ignores viewport and the 300-point render cap.
  // Normal mode remains viewport-only and capped at 300.
